...
class SchemaDriftMonitor:
    ...
    def _categorize_fields(self, gdf: gpd.GeoDataFrame) -> Dict[str, List[str]]:
        ...
        for col in gdf.columns:
            ...
            elif col in ['TOTAL', 'DEM', 'REP', 'NAV', 'OTH', 'IND', 'LBT', 'WFP', 'WTP', 'CON', 'NLB', 'PGP', 'PRO']:
                categories["registration_counts"].append(col)
            elif col.startswith('reg_pct_'):
                categories["registration_percentages"].append(col)
            elif col in ['OR_House', 'OR_Senate', 'USCongress', 'CITY', 'SchoolDist', 'FIRE_DIST', 'TRAN_DIST', 'WaterDist', 'SewerDist', 'PUD', 'ESD', 'METRO', 'Mult_Comm', 'CommColleg', 'CoP_Dist', 'Soil_Water', 'UFSWQD', 'Unincorp']:
            ...
            elif col in ['margin_pct', 'total_votes', 'zone1_vote_share', 'precinct_size', 'competitiveness_score', 'engagement_rate', 'swing_potential']:
    ...
    def capture_schema_snapshot(self, gdf: gpd.GeoDataFrame, data_source: str = "election_data") -> SchemaSnapshot:
        ...
        logger.info(f"Schema snapshot captured: {snapshot.total_fields} fields, hash: {snapshot.schema_hash}")
        return snapshot
    ...
    def analyze_schema_drift(self, current_snapshot: SchemaSnapshot) -> List[SchemaDriftAlert]:
        ...
        type_changes = self._find_type_changes(previous_data["field_types"], current_snapshot.field_types)
        if type_changes:
    ...
    def _find_type_changes(self, previous_types: Dict[str, str], current_types: Dict[str, str]) -> Dict[str, Tuple[str, str]]:
    ...
    def _assess_new_fields_impact(self, new_fields: Set[str], snapshot: SchemaSnapshot) -> str:
        ...
        for category, fields in categories.items():
            ...
            if new_in_category:
                impact_parts.append(f"{len(new_in_category)} new {category.replace('_', ' ')} fields")
    ...
    def _assess_removed_fields_impact(self, removed_fields: Set[str]) -> str:
        ...
        critical_removed = [f for f in removed_fields if any(f.startswith(p) for p in critical_patterns)]
    ...
    def _recommend_new_fields_actions(self, new_fields: Set[str]) -> List[str]:
        ...
        if len(new_fields) > 5:
            actions.append("Consider implementing auto-registration patterns for bulk field additions")
    ...
    def _recommend_type_changes_actions(self, type_changes: Dict[str, Tuple[str, str]]) -> List[str]:
    ...
    def _analyze_data_quality_changes(self, previous_data: Dict, current_snapshot: SchemaSnapshot) -> List[SchemaDriftAlert]:
        """Analyze data quality changes between snapshots."""
        alerts = []
        
        # Check for significant changes in null rates
        for field in set(previous_data["null_counts"].keys()) & set(current_snapshot.null_counts.keys()):
            ...
            if null_rate_change > self.config["alert_thresholds"]["null_rate_change_threshold"]:
                alert = SchemaDriftAlert(
                    ...
                    impact_assessment=f"Data quality change may indicate upstream issues or different data collection",
                    recommended_actions=["Investigate source of null rate change", "Review data validation rules"]
                )
                alerts.append(alert)
        
        # Check for significant record count changes
        record_count_change = abs(current_snapshot.record_count - previous_data["record_count"]) / previous_data["record_count"]
        if record_count_change > self.config["alert_thresholds"]["record_count_change_threshold"]:
            ...
            alert = SchemaDriftAlert(
                ...
                title=f"Significant Record Count Change",
                ...
                recommended_actions=["Verify data source completeness", "Check for filtering or processing changes"]
            )
            alerts.append(alert)
    ...
    def generate_drift_report(self, days_back: int = 30) -> str:
        ...
        recent_snapshots = [s for s in snapshots if datetime.fromisoformat(s["timestamp"]) > cutoff_date]
        recent_alerts = [a for a in alerts if datetime.fromisoformat(a["timestamp"]) > cutoff_date]
        
        # Generate report
        report_lines = [
            f"# Schema Drift Monitoring Report",
            f"",
        ...
        if len(recent_alerts) == 0:
            report_lines.append("✅ **Schema Stable:** No significant drift detected. Continue monitoring.")
    ...
    def get_alert_summary(self, severity_filter: Optional[str] = None, days_back: int = 7) -> Dict[str, Any]:
...
def monitor_schema_drift(gdf: gpd.GeoDataFrame, data_source: str = "election_data") -> Dict[str, Any]:
    """
