...
for metric in ['ela', 'math', 'science']:
    ax1.plot(df['year'], df[metric],
             marker='o', linewidth=2, label=subject_labels[metric],
             color=subject_colors[metric])

    # Plot imputed data explicitly
    imputed_mask = df_imputed[metric].values
    ax1.scatter(df['year'][imputed_mask], df[metric][imputed_mask],
                facecolors='none', edgecolors=subject_colors[metric],
                marker='*', s=300, linewidth=2, label=f'Imputed {subject_labels[metric]}')

# Plot graduation on secondary axis
ax2.plot(df['year'], df['graduation'],
...
ax2.scatter(df['year'][imputed_grad_mask], df['graduation'][imputed_grad_mask],
            facecolors='none', edgecolors=subject_colors['graduation'],
