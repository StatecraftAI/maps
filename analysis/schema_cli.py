...
def configure_monitoring(args: argparse.Namespace) -> None:
    ...
    if args.show:
        print(f"📋 Current Monitoring Configuration:")
...
def status(args: argparse.Namespace) -> None:
    ...
    print(f"📊 Schema Drift Monitoring Status")
    print("=" * 50)
    ...
    print(f"\n⚙️ Configuration:")
    print(f"  • Monitoring Enabled: {'✅' if monitor.config.get('monitoring_enabled', True) else '❌'}")
...
Examples:
  ...
  %(prog)s status
    ...
    alerts_parser.add_argument('--severity', type=str, choices=['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'],
    ...
    analyze_parser.add_argument('--source', type=str, 
                               help='Data source identifier (default: filename)')
    ...
    config_group.add_argument('--set', type=str, metavar='KEY=VALUE',
