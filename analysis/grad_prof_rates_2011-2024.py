...
imputed_years = df.loc[df[['ela', 'math', 'science', 'graduation']].isna().any(axis=1), 'year'].tolist()
...
ax.plot(df['year'], df['avg_proficiency'], label='Avg. Proficiency (ELA, Math, Science)',
        color="#D55E00", linewidth=3, marker='o')

# Plot Graduation Rates
ax.plot(df['year'], df['graduation'], label='Graduation Rate',
        color="#0072B2", linewidth=3, marker='o', linestyle='--')
...
for year in imputed_years:
    ax.scatter(df.loc[df['year'] == year, 'year'], df.loc[df['year'] == year, 'avg_proficiency'],
               facecolors='none', edgecolors='black', marker='s', s=250, linewidth=2)
