  ...
  python run_pipeline.py --demographics-only            # Only run demographic analysis
    ...
    if not args.skip_tiles:
        if not check_tippecanoe():
            ...
            print("   Install with: brew install tippecanoe (macOS) or sudo apt install gdal-bin (Ubuntu)")
            args.skip_tiles = True
        else:
    ...
    # Pipeline execution
    pipeline_name = "Demographics Analysis" if args.demographics_only else "Election Data Processing Pipeline"
    print(f"\n🗺️ {pipeline_name}")
    ...
    if not args.skip_tiles:
        ...
        if run_script(TILES_SCRIPT, "Vector Tile Creation"):
            ...
            print("   💡 Try running just the mapping step with: python run_pipeline.py --skip-tiles")
            sys.exit(1)
    else:
