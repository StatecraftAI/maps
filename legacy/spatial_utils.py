...
class SpatialProcessor:
    ...
    def standardize_crs(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        if current_crs != target_crs:
            ...
            try:
                ...
                logger.success("  ✅ CRS transformation completed")
            except Exception as e:
    ...
    def optimize_properties(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        for col in gdf.columns:
            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(series) and series.dtype in [
                    "int8",
                    "int16",
                    "float32",
                ]:
        ...
        logger.info(
            f"  💾 Memory usage: {original_memory:,} → {final_memory:,} bytes ({memory_reduction:.1f}% reduction)"
        )
                ...
                if numeric_series.dtype == "float64":
                    ...
                    if numeric_series.dropna().apply(lambda x: x.is_integer()).all():
                        ...
                        if -128 <= min_val <= 127 and -128 <= max_val <= 127:
                            ...
                            -2147483648 <= min_val <= 2147483647
                            and -2147483648 <= max_val <= 2147483647
                        ):
    ...
    def generate_metadata(
        self, gdf: gpd.GeoDataFrame, table_name: str, description: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def export_geojson(self, gdf: gpd.GeoDataFrame, output_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            gdf.to_file(output_path, driver="GeoJSON", coordinate_precision=precision)
    ...
    def process(
        ...
        logger.info(
            f"  📊 Features: {self.stats['input_features']:,} → {self.stats['output_features']:,}"
        )
...
Examples:
  ...
  python process_geojson_universal.py input.geojson --table my_table --simplify 0.001 --bounds -122.8 45.4 -122.4 45.7
    ...
    # Processing options
    parser.add_argument(
        "--precision", type=int, default=6, help="Coordinate precision (decimal places)"
    )
    parser.add_argument(
        "--clip-to-pps", action="store_true", help="Clip features to PPS district boundaries"
    )
    parser.add_argument(
        "--bounds",
        nargs=4,
    ...
    parser.add_argument(
        "--remove-invalid", action="store_true", help="Remove invalid geometries instead of fixing"
    )
    parser.add_argument(
        "--target-crs", default="EPSG:4326", help="Target coordinate reference system"
    )
    ...
    try:
        ...
        if gdf is not None:
            ...
            print("=" * 50)
            print(
                f"Input features:     {metadata.get('processing_stats', {}).get('input_features', 0):,}"
            )
            print(f"Output features:    {metadata.get('feature_count', 0):,}")
            print(
                f"Processing time:    {metadata.get('processing_stats', {}).get('processing_time', 0):.2f}s"
            )
...
def load_and_validate_voter_data(config: Config) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        valid_coords = (df["Latitude"].between(45.0, 46.0)) & (
...
def create_hexagonal_aggregation(
    ...
    hex_stats = (
        voters_gdf.groupby("h3_index")
        .agg({"VOTER_ID": "count"})
        .rename(columns={"VOTER_ID": "voter_count"})
        .reset_index()
    )
...
def load_pps_district_boundaries(config: Config) -> Optional[gpd.GeoDataFrame]:
    ...
    try:
        ...
        logger.success("  ✅ Loaded PPS district boundaries")
...
def classify_by_spatial_join(
    points_gdf: gpd.GeoDataFrame,
    polygons_gdf: gpd.GeoDataFrame,
    classification_col: str = "within_polygon",
) -> gpd.GeoDataFrame:
    """
    ...
    """
    logger.info("🎯 Classifying points by polygon containment...")
    ...
    try:
        ...
        points_with_classification[
            classification_col
        ] = ~points_with_classification.index_right.isna()
        ...
        logger.info(
            f"     📊 Coverage: {within_count / len(points_with_classification) * 100:.1f}%"
        )
...
def create_grid_aggregation(
    points_gdf: gpd.GeoDataFrame, grid_size: float = 0.01, count_col: str = "point_count"
) -> gpd.GeoDataFrame:
    """
...
def analyze_points_by_polygons(
    ...
    """
    logger.info("🏘️ Analyzing point distribution by polygons...")
    ...
    try:
        ...
        if polygon_id_col in points_with_polygons.columns:
            polygon_stats = (
                points_with_polygons.groupby(polygon_id_col)
                .agg({point_id_col: "count"})
                .rename(columns={point_id_col: count_col})
                .reset_index()
            )
