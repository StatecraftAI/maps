...
@click.option('--zone', type=str, metavar='ZONE', help='Quick zone switching (zone number or election name, e.g., 4 or bond)')
@click.option('--votes-csv', type=click.Path(exists=False), help='Override input votes CSV file path')
...
@click.option('--trace', is_flag=True, help='Enable TRACE level logging for deep debugging (maximum detail)')
...
def cli(ctx, **kwargs):
    ...
    setup_logging(
        ...
        enable_trace=kwargs.get('trace', False)
    )
    
    # Add file logging if requested
    if kwargs.get('log_file'):
        log_file = kwargs['log_file']
        log_level = "TRACE" if kwargs.get('trace') else ("DEBUG" if kwargs.get('verbose') else "INFO")
        logger.add(
            log_file,
            level=log_level,
...
def run_pipeline(ctx):
    ...
    try:
        ...
        pipeline_name = (
            "Demographics Analysis" if kwargs['demographics_only'] else "Election Data Processing Pipeline"
        )
        ...
        if kwargs['include_demographics'] and demographic_files_available:
            try:
                ...
                if acs_json_path.exists():
                    ...
                    logger.info(f"⏭️ Skipping household demographics analysis ({acs_json_path.name} not found)")
            except Exception as e:
        ...
        if success_count == total_steps:
            ...
            if kwargs['include_demographics'] or kwargs['demographics_only']:
                try:
                    ...
                    logger.info("   🏠 Household demographics: data/maps/household_demographics.html")
                    logger.info("   📊 Demographics data: data/")
            
            ctx.exit(0)
        else:
            logger.warning(f"Pipeline completed with issues ({success_count}/{total_steps} successful)")
            ctx.exit(1)
...
def run_script(script_path: Path, description: str) -> bool:
    ...
    try:
        ...
        subprocess.run([sys.executable, str(script_path)], 
                      cwd=script_path.parent, check=True, capture_output=False, env=env)
