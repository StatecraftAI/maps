...
from data_utils import load_json_array, load_geojson, log_data_info
...
def load_and_process_acs_data(config) -> pd.DataFrame:
    ...
    try:
        ...
        # Convert to numeric
        acs_df["total_households"] = pd.to_numeric(acs_df["total_households"], errors="coerce").fillna(0).astype(int)
        acs_df["households_no_minors"] = pd.to_numeric(acs_df["households_no_minors"], errors="coerce").fillna(0).astype(int)
