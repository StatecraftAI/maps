...
def calculate_competitiveness(
    ...
    if isinstance(margin_pct, pd.Series):
        result = pd.cut(abs_margin, bins=[0] + thresholds + [float('inf')], 
...
def calculate_turnout_rate(
    ...
    """
    return calculate_percentage(votes_cast, registered_voters, handle_zero=True, round_digits=round_digits)
...
def calculate_vote_share(
    ...
    """
    return calculate_percentage(candidate_votes, total_votes, handle_zero=True, round_digits=round_digits)


def find_leading_candidate(
...
def calculate_contribution_percentage(
    ...
    """
    return calculate_percentage(part_values, total_value, handle_zero=True, round_digits=round_digits)
...
def normalize_to_range(
    ...
    if isinstance(values, pd.Series):
        ...
        return [((v - values_min) / (values_max - values_min)) * (max_val - min_val) + min_val for v in values]
...
def calculate_election_metrics(
    ...
    result_df["vote_margin"] = calculate_vote_margin(votes_df, candidate_cols, margin_type="absolute")
    
    # Calculate margin percentage
    if total_col in votes_df.columns:
        result_df["margin_pct"] = calculate_percentage(
            result_df["vote_margin"], 
            result_df[total_col], 
            handle_zero=True
        )
