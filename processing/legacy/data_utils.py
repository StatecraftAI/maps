    ...
    **kwargs
    ...
    try:
        ...
        if filter_county and "STATEFP" in gdf.columns and "COUNTYFP" in gdf.columns:
            ...
            logger.info(f"    Filtered to county {filter_county}: {len(gdf):,} features (was {original_count:,})")
...
def load_json(file_path: Union[str, Path]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.success(f"  ✅ Loaded JSON data")
        return data
...
def validate_required_columns(df: pd.DataFrame, required_cols: List[str], data_name: str = "data") -> bool:
    """
...
def clean_coordinate_columns(df: pd.DataFrame, lat_col: str = "Latitude", lon_col: str = "Longitude") -> pd.DataFrame:
    """
    ...
    valid_coords = (
        (df[lat_col].between(45.0, 46.0)) & 
        (df[lon_col].between(-123.5, -122.0))
    )
...
def log_data_info(data: Union[pd.DataFrame, gpd.GeoDataFrame], data_name: str):
    ...
    if isinstance(data, gpd.GeoDataFrame):
        ...
        logger.info(f"    Geometry type: {data.geometry.geom_type.iloc[0] if len(data) > 0 else 'None'}")
...
def _validate_coordinates(df: pd.DataFrame, coord_bounds: Optional[Dict[str, float]] = None) -> pd.DataFrame:
    """Validate coordinate columns with optional custom bounds."""
    if coord_bounds is None:
        # Default Oregon bounds
        coord_bounds = {
            'lat_min': 45.0, 'lat_max': 46.0,
            'lon_min': -123.5, 'lon_max': -122.0
        }
