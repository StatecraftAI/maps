...
def process_household_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # After sanitization, columns are: name, b11001_001e (total households), b11001_002e (households no minors), state, county, tract, block_group
    df = df.rename(
        columns={
    ...
    df["pct_households_no_minors"] = np.where(
        df["total_households"] > 0, (df["households_no_minors"] / df["total_households"]) * 100, 0
    )

    logger.info(f"  ✅ Calculated metrics for {len(df)} block groups")
    logger.info(
        f"  📊 Avg % households without minors: {df['pct_households_no_minors'].mean():.1f}%"
    )
...
def merge_acs_with_boundaries(df: pd.DataFrame, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    logger.info(
        f"  📊 Data coverage: {has_data.sum()}/{len(merged_gdf)} block groups have household data"
    )
...
def filter_to_pps_district(gdf: gpd.GeoDataFrame, config: Config) -> gpd.GeoDataFrame:
    ...
    logger.info(
        f"  ✅ Filtered: {len(pps_gdf_filtered)}/{len(gdf)} block groups within PPS district"
    )
...
def add_demographic_analysis(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    gdf["household_density"] = np.where(
        gdf["area_sq_km"] > 0, gdf["total_households"] / gdf["area_sq_km"], 0
    )
    ...
    if len(valid_density) > 0:
        ...
        gdf.loc[
            mask & (gdf["household_density"] > q1) & (gdf["household_density"] <= q3),
            "density_category",
        ] = "Medium Density"
        gdf.loc[mask & (gdf["household_density"] > q3), "density_category"] = "High Density"

        logger.info(
            f"  ✅ Added density_category (Low: ≤{q1:.0f}, Med: {q1:.0f}-{q3:.0f}, High: >{q3:.0f})"
        )
    ...
    gdf.loc[
        mask & (gdf["pct_households_no_minors"] >= 50) & (gdf["pct_households_no_minors"] < 75),
        "family_composition",
    ] = "Mixed"
...
def prepare_households_data() -> str:
    ...
    gdf = clean_and_validate(gdf, "household")

    # Save for upload
    output_file = ensure_output_directory("../data/processed/processed_households_data.geojson")
